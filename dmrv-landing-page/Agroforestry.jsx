...
import {
  ...
  Wheat, Trees, Map, Scan, Award, HeartHandshake,
} from "lucide-react";
...
function Navbar() {
  ...
  const links = [["Overview","overview"],["Platform","platform"],["Dashboard","dashboard"],["Money Market","carbon-credits"],["Why Emertech","why-emertech"]];
  return (
...
function ContactForm() {
  ...
  return (
    <div id="contact" style={{ background:C.white, borderRadius:20, overflow:"hidden", boxShadow:"0 8px 40px rgba(124,58,237,0.14)", border:`1px solid ${C.pillBorder}`, scrollMarginTop:90 }}>
      <div style={{ background:grad, padding:"1.35rem 1.6rem" }}>
        ...
        <p style={{ fontFamily:"'Inter', sans-serif", color:"rgba(255,255,255,0.82)", fontSize:"0.8rem", marginTop:4, marginBottom:0 }}>See how Emertech can power your agroforestry MRV reporting.</p>
      </div>
      <div style={{ padding:"1.35rem 1.6rem" }}>
        {done ? (
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
            ...
            <textarea style={{...inp, minHeight:78, resize:"none"}} placeholder="Tell us about your agroforestry / reforestation project" value={d.msg} onChange={set("msg")} onFocus={focus} onBlur={blur}/>
            ...
            <button onClick={submit} style={{ background:grad, color:"#fff", border:"none", borderRadius:10, fontFamily:"'Inter', sans-serif", fontWeight:600, fontSize:"0.86rem", padding:"0.82rem", cursor:"pointer", width:"100%" }}>Request a demo</button>
...
function Hero() {
  const checks = ["Satellite & LiDAR-verified biomass","Verra, Gold Standard & Plan Vivo methodologies","Transparent direct-to-farmer payouts","End-to-end forest carbon traceability"];
  return (
    <section id="overview" style={{ paddingTop:70, background:"#ffffff", backgroundImage:"radial-gradient(circle, rgba(168,85,247,0.06) 1px, transparent 1px)", backgroundSize:"26px 26px", position:"relative", overflow:"hidden" }}>
      ...
      <div style={{ maxWidth:1180, margin:"0 auto", display:"grid", gridTemplateColumns:"1.7fr 1fr", gap:"3rem", alignItems:"start", padding:"5.5rem 2rem 7rem", position:"relative" }}>
        <div>
          <Pill dot>Agroforestry dMRV — Live</Pill>
          <H as="h1" style={{ marginTop:22, fontSize:"2.85rem", lineHeight:1.1 }}>
            Precision Intelligence for <span style={{ background:grad, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>Nature-Based Solutions.</span>
          </H>
          <Body style={{ marginTop:18, fontSize:"0.95rem" }}>Leverage AI, satellite data, and mobile ground-truthing to originate, monitor, and issue high-integrity carbon credits for agroforestry and reforestation projects.</Body>
          ...
          <div style={{ marginTop:24, display:"flex", alignItems:"center", gap:10, flexWrap:"wrap" }}>
            <Btn onClick={go("contact")}>Request Demo <ArrowRight style={{ width:15, height:15 }}/></Btn>
            <button onClick={go("dashboard")} style={{ background:C.white, color:C.primaryDark, border:`1px solid ${C.pillBorder}`, borderRadius:9999, fontFamily:"'Inter', sans-serif", fontWeight:600, fontSize:"0.88rem", padding:"0.6rem 1.5rem", cursor:"pointer", display:"inline-flex", alignItems:"center", gap:6, transition:"background .18s, border-color .18s" }}
              onMouseEnter={e=>{e.currentTarget.style.background=C.iconBg;e.currentTarget.style.borderColor=C.primary;}}
              onMouseLeave={e=>{e.currentTarget.style.background=C.white;e.currentTarget.style.borderColor=C.pillBorder;}}>Explore Methodologies</button>
          </div>
        </div>
        <ContactForm/>
      </div>
...
function ValueProp() {
  const cols = [
    { icon:Layers,    title:"Transparency across every hectare", desc:"Emertech enables full visibility into your agroforestry project. Track canopy, biomass, and species composition at every stage — from baseline through growth and verified sequestration." },
    { icon:Globe,     title:"Simplify sustainability reporting",  desc:"Streamline Verra, Gold Standard, and Plan Vivo reporting with accurate, verifiable data from satellite, LiDAR, and the field, reducing the effort to compile submissions." },
    { icon:BadgeCheck,title:"Achieve your compliance goals",      desc:"Gain actionable insights into your project portfolio, align operations with high-integrity carbon standards, and empower your team to make data-driven decisions." },
  ];
  return (
    <section style={{ background:C.white, padding:"5rem 2rem" }}>
      <div style={{ maxWidth:1180, margin:"0 auto" }}>
        <FadeUp style={{ textAlign:"center" }}>
          <Eyebrow>For project developers</Eyebrow>
          <H as="h2" style={{ fontSize:"2.2rem", marginTop:12, whiteSpace:"nowrap" }}>Unlock compliance with verifiable forest carbon data</H>
        </FadeUp>
        ...
        <FadeUp delay={120}>
          <div style={{ marginTop:44, display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:16, background:C.sectionAlt, border:`1px solid #e5e7eb`, borderRadius:18, padding:"1.4rem 1.8rem" }}>
            <div><Eyebrow>Ready to get started?</Eyebrow><H as="p" style={{ fontSize:"1.05rem", marginTop:4 }}>Partner with Emertech to implement verified agroforestry MRV.</H></div>
...
================================================================ */
const SOLUTIONS = [
  { icon:Trees,      tag:"CORE PRODUCT", title:"Biomass & carbon tracking",    desc:"Quantify above-ground biomass and sequestered carbon using satellite, LiDAR, and field sampling against approved registry methodologies." },
  { icon:Activity,   tag:"TRACEABILITY",  title:"Lifecycle traceability",       desc:"Track project activity from baseline and planting through canopy growth, sequestration, and credit issuance with a full provenance record." },
  { icon:TrendingUp, tag:"PERFORMANCE",   title:"Sequestration modeling",       desc:"Real-time modeling of carbon removals and project-level GHG impact across every parcel, cohort, and crediting period." },
  { icon:Scan,       tag:"REMOTE SENSING",title:"AI canopy analysis",           desc:"Machine-learning models on satellite and LiDAR data measure canopy density, tree height, and species distribution at scale." },
  { icon:HeartHandshake,tag:"CO-BENEFITS",title:"Community & ESG tracking",     desc:"Full visibility into biodiversity metrics and transparent, direct-to-farmer payouts that keep project equity verifiable." },
  { icon:ShieldCheck,tag:"COMPLIANCE",    title:"Immutable audit trail",        desc:"Cryptographic records ensure tamper-proof documentation for registry audits and third-party validation." },
];
    ...
    <section id="platform" style={{ background:C.sectionAlt, padding:"5rem 2rem", scrollMarginTop:70 }}>
      <div style={{ maxWidth:1180, margin:"0 auto" }}>
        <FadeUp style={{ textAlign:"center" }}>
          ...
          <H as="h2" style={{ fontSize:"2.4rem", marginTop:12 }}>A complete agroforestry MRV platform</H>
          <Body style={{ marginTop:10, maxWidth:460, margin:"10px auto 0" }}>Everything you need for forest carbon origination, monitoring, and issuance, in one end-to-end workflow.</Body>
        </FadeUp>
...
const DEMO_TABS = [
  { icon:Map,           title:"Land Tenure & Polygon Mapping", desc:"Digitize project boundaries, verify land ownership, and establish historical baselines using high-res satellite imagery." },
  { icon:Scan,          title:"AI Canopy & LiDAR Analysis",    desc:"Deploy machine-learning models on satellite and LiDAR data to measure tree height, canopy density, and species distribution." },
  { icon:Trees,         title:"Biomass & Carbon Estimation",   desc:"Automatically calculate Above-Ground Biomass (AGB) and sequestered carbon using localized allometric equations." },
  { icon:HeartHandshake,title:"Community & ESG Tracking",      desc:"Track co-benefits, biodiversity metrics, and transparent, direct-to-farmer financial payouts to ensure project equity." },
  { icon:Award,         title:"Registry-Grade Issuance",       desc:"Generate automated, audit-ready reports formatted strictly for Verra (e.g. VM0047), Gold Standard, and Plan Vivo." },
];
...
/* Mock 1 → Community & ESG Tracking */
function MockProfiling() {
  const initial = [
    { name:"Cooperative A · Nashik", amount:450, paid:true },
    { name:"Anita Devi · Sambalpur", amount:280, paid:true },
    { name:"Guntur Growers Group",   amount:610, paid:true },
    { name:"Warangal Smallholders",  amount:350, paid:false },
    { name:"Ludhiana Agroforest Co", amount:840, paid:false },
  ];
  const [rows, setRows] = useState(initial);
  const paidCount = rows.filter(r => r.paid).length;
  const disbursed = rows.filter(r => r.paid).reduce((s,r)=>s+r.amount,0);
  ...
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
        <p style={{ fontFamily:"'Inter', sans-serif", fontWeight:700, fontSize:"0.92rem", color:C.h, margin:0 }}>Community Payouts & Co-Benefits</p>
        <SmallPill>{paidCount}/{rows.length} PAID</SmallPill>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:14 }}>
        <div style={{ background:grad, borderRadius:12, padding:"0.85rem 1rem", color:"#fff", boxShadow:"0 6px 18px rgba(124,58,237,0.28)" }}>
          <p style={{ fontFamily:"'Inter', sans-serif", fontSize:"0.64rem", textTransform:"uppercase", letterSpacing:"0.08em", color:"rgba(255,255,255,0.82)", margin:0 }}>Recent Payout</p>
          <p style={{ fontFamily:"'Inter', sans-serif", fontWeight:700, fontSize:"1.25rem", margin:"3px 0 0" }}>$450</p>
          <p style={{ fontFamily:"'Inter', sans-serif", fontSize:"0.66rem", color:"rgba(255,255,255,0.82)", margin:"2px 0 0" }}>to Cooperative A</p>
        </div>
        <div style={{ background:"#f0fdf4", border:"1px solid #bbf7d0", borderRadius:12, padding:"0.85rem 1rem" }}>
          <div style={{ display:"flex", alignItems:"center", gap:6, marginBottom:4 }}>
            <Leaf style={{ color:"#15803d", width:14, height:14 }}/>
            <span style={{ fontFamily:"'Inter', sans-serif", fontSize:"0.66rem", textTransform:"uppercase", letterSpacing:"0.08em", color:"#9ca3af" }}>Biodiversity Index</span>
          </div>
          <p style={{ fontFamily:"'Inter', sans-serif", fontWeight:700, fontSize:"1.05rem", color:"#15803d", margin:0 }}>● Stable</p>
          <p style={{ fontFamily:"'Inter', sans-serif", fontSize:"0.66rem", color:C.body, margin:"2px 0 0" }}>${disbursed.toLocaleString()} disbursed total</p>
        </div>
      </div>
      <table style={{ width:"100%", borderCollapse:"collapse" }}>
        <thead><tr><th style={th}>Recipient</th><th style={{...th, textAlign:"right"}}>Amount</th><th style={{...th, textAlign:"right"}}>Status</th></tr></thead>
        <tbody>
          {rows.map((r,realIdx) => (
            <tr key={r.name}>
              ...
              <td style={{...td, textAlign:"right"}}>
                {r.paid
                  ? <StatusPill ok>Paid</StatusPill>
                  : <button onClick={()=>setRows(rs=>rs.map((x,i)=>i===realIdx?{...x,paid:true}:x))} style={{ cursor:"pointer", border:`1px solid #fde68a`, background:"#fffbeb", color:"#92400e", borderRadius:9999, padding:"0.2rem 0.65rem", fontFamily:"'Inter', sans-serif", fontWeight:700, fontSize:"0.68rem" }}>Release payout →</button>}
              </td>
            </tr>
          ))}
        </tbody>
  ...
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10, flexWrap:"wrap", gap:8 }}>
        <p style={{ fontFamily:"'Inter', sans-serif", fontWeight:700, fontSize:"0.92rem", color:C.h, margin:0 }}>Project Boundary & Tenure</p>
...
/* Mock 4 → Biomass & Carbon Estimation */
function MockChart() {
  const [area, setArea] = useState(50);
  const [rate, setRate] = useState(8);
  const baseRate = 0.5;
  const years = [2026,2031,2036,2041,2046];
  const data = years.map((yr,i) => ({ yr, base: area*baseRate, proj: area*baseRate*(1 + (rate/100)*(i*4)) }));
  const max = Math.max(...data.map(d=>d.proj), 1);
  const uplift = Math.round(((data[4].proj - data[4].base) / data[4].base) * 100);
  return (
    <div>
      <div style={{ background:"#fafafa", border:`1px solid ${C.cardBorder}`, borderRadius:10, padding:"0.8rem 1rem", marginBottom:14 }}>
        <Slider label="Project area" value={area} min={5} max={500} step={5} unit=" ha" onChange={setArea}/>
        <Slider label="Annual biomass growth rate" value={rate} min={2} max={20} step={1} unit="% / yr" onChange={setRate}/>
      </div>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:6 }}>
        <p style={{ fontFamily:"'Inter', sans-serif", fontWeight:700, fontSize:"0.88rem", color:C.h, margin:0 }}>Baseline vs Sequestration (tCO₂e)</p>
        <div style={{ display:"flex", gap:12, fontFamily:"'Inter', sans-serif", fontSize:"0.66rem", color:C.body }}>
          ...
          <span style={{ display:"inline-flex", alignItems:"center", gap:5 }}><span style={{ width:9, height:9, borderRadius:2, background:C.primary }}/>Projected AGB</span>
        </div>
      </div>
        ...
        {data.map(d => (
          <div key={d.yr} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:7, flex:1 }}>
            <div style={{ display:"flex", alignItems:"flex-end", gap:5, height:120 }}>
              <div style={{ width:14, height:`${(d.base/max)*100}%`, background:"#e5e7eb", borderRadius:"4px 4px 0 0", transition:"height .3s" }}/>
              <div style={{ width:14, height:`${(d.proj/max)*100}%`, background:grad, borderRadius:"4px 4px 0 0", boxShadow:"0 2px 8px rgba(124,58,237,0.25)", transition:"height .3s" }}/>
            </div>
            <span style={{ fontFamily:"'Inter', sans-serif", fontSize:"0.68rem", fontWeight:600, color:C.body }}>{d.yr}</span>
          </div>
        ))}
      </div>
      <p style={{ fontFamily:"'Inter', sans-serif", fontSize:"0.78rem", color:C.body, marginTop:12, marginBottom:0 }}>
        Over a 20-year crediting period, projected Above-Ground Biomass outpaces baseline by <strong style={{ color:C.primary }}>+{uplift}%</strong> ({Math.round(data[4].proj)} tCO₂e/yr) under VM0047 allometric estimation.
      </p>
    </div>
  );
}

/* Mock 5 → AI Canopy & LiDAR Analysis */
function MockMonitoring() {
  const [canopy, setCanopy] = useState(84);
  const [height, setHeight] = useState(14);
  const [sync, setSync] = useState("2 mins ago");
  const status = canopy < 40 ? { t:"Sparse cover", c:"#b45309" } : canopy > 90 ? { t:"Dense / closed", c:"#1d4ed8" } : { t:"Healthy canopy", c:"#15803d" };
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
        <p style={{ fontFamily:"'Inter', sans-serif", fontWeight:700, fontSize:"0.92rem", color:C.h, margin:0 }}>Canopy & LiDAR Scan</p>
        <span style={{ display:"inline-flex", alignItems:"center", gap:5, fontFamily:"'Inter', sans-serif", fontSize:"0.7rem", fontWeight:600, color:"#7c3aed" }}>
          <span style={{ position:"relative", display:"inline-flex", width:8, height:8 }}>
            <span style={{ position:"absolute", inset:0, borderRadius:"50%", background:"#7c3aed", opacity:.7, animation:"ping 1.5s cubic-bezier(0,0,.2,1) infinite" }}/>
            <span style={{ position:"relative", width:8, height:8, borderRadius:"50%", background:"#7c3aed", display:"block" }}/>
          </span>Scanning
        </span>
      </div>
      <div style={{ background:"#fafafa", border:`1px solid ${C.cardBorder}`, borderRadius:10, padding:"0.8rem 1rem", marginBottom:14 }}>
        <Slider label="Canopy cover" value={canopy} min={0} max={100} step={1} unit="%" onChange={setCanopy}/>
        <Slider label="Avg tree height" value={height} min={1} max={40} step={1} unit=" m" onChange={setHeight}/>
        <button onClick={()=>setSync("just now")} style={{ cursor:"pointer", border:`1px solid ${C.pillBorder}`, background:C.iconBg, color:C.primary, borderRadius:9, padding:"0.45rem 0.9rem", fontFamily:"'Inter', sans-serif", fontWeight:700, fontSize:"0.74rem", display:"inline-flex", alignItems:"center", gap:6 }}><RotateCcw style={{ width:13, height:13 }}/> Re-run scan</button>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
        {[
          { icon:Scan,        label:"Canopy Cover",   value:`${canopy}%`, note:status.t, nc:status.c },
          { icon:Trees,       label:"Avg Tree Height",value:`${height}.2 m`, note:"LiDAR-derived", nc:"#9ca3af" },
          { icon:Leaf,        label:"Species Detected",value:"7", note:"ML classification", nc:"#9ca3af" },
          { icon:Satellite,   label:"Latest Pass",    value:sync, note:"Sentinel-2 + LiDAR", nc:"#9ca3af" },
        ].map(m => {
          const Icon = m.icon;
          return (
...
function MockReporting() {
  const allDocs = [
    ["Project_Design_Document_VM0047.pdf", "2.4 MB"],
    ["Third-Party_Validation_Report.pdf", "1.1 MB"],
    ["Verified_Carbon_Statement.pdf", "880 KB"],
  ];
  ...
  return (
    <div>
      <p style={{ fontFamily:"'Inter', sans-serif", fontWeight:700, fontSize:"0.92rem", color:C.h, margin:"0 0 12px" }}>Registry Document Generator</p>
      ...
      {phase==="running" && (
        <div style={{ background:"#fafafa", border:`1px solid ${C.cardBorder}`, borderRadius:10, padding:"0.9rem 1.1rem" }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:9 }}>
            <span style={{ fontFamily:"'Inter', sans-serif", fontWeight:600, fontSize:"0.82rem", color:C.strong }}>Validating VM0047 parameters…</span>
...
const MOCKS = [MockMap, MockMonitoring, MockChart, MockProfiling, MockReporting];
    ...
    <section id="dashboard" style={{ background:C.white, padding:"5rem 2rem", scrollMarginTop:70 }}>
      ...
      <div style={{ maxWidth:1180, margin:"0 auto" }}>
        <FadeUp style={{ textAlign:"center", marginBottom:48 }}>
          <Eyebrow>The Agroforestry MRV Switcher</Eyebrow>
          <H as="h2" style={{ fontSize:"2.4rem", marginTop:12 }}>One platform, the full forest carbon lifecycle</H>
          <Body style={{ marginTop:10, maxWidth:540, margin:"10px auto 0" }}>Click through the workflow — map a boundary, run a canopy scan, and model 20-year sequestration live.</Body>
        </FadeUp>
...
function StatsBar() {
  return (
    <section style={{ background:C.sectionAlt, padding:"3.5rem 2rem" }}>
      <div style={{ maxWidth:1180, margin:"0 auto" }}>
        <FadeUp>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:20, background:C.white, border:`1px solid ${C.cardBorder}`, borderRadius:20, padding:"2.2rem", boxShadow:"0 4px 20px rgba(124,58,237,0.06)" }}>
            {STATS.map(([n,l],i) => (
              <div key={l} style={{ textAlign:"center", borderRight: i<STATS.length-1?`1px solid ${C.cardBorder}`:"none" }}>
                <p style={{ fontFamily:"'Inter', sans-serif", fontWeight:700, fontSize:"2.4rem", letterSpacing:"-0.02em", margin:0, background:grad, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>{n}</p>
                <p style={{ fontFamily:"'Inter', sans-serif", fontWeight:600, fontSize:"0.86rem", color:C.body, margin:"6px 0 0" }}>{l}</p>
              </div>
            ))}
          </div>
        </FadeUp>
      ...
      <div style={{ maxWidth:1180, margin:"0 auto" }}>
        <FadeUp>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"3.5rem", alignItems:"center" }}>
            <div style={{ borderRadius:20, overflow:"hidden", border:`1px solid ${C.cardBorder}`, boxShadow:"0 16px 48px rgba(124,58,237,0.14)", position:"relative", aspectRatio:"600 / 400", background:"#3a4a28" }}>
              {/* Stylised forest-canopy / satellite-analysis placeholder (purple analysis overlay, no green accents on UI chrome) */}
              <svg viewBox="0 0 600 400" preserveAspectRatio="none" style={{ position:"absolute", inset:0, width:"100%", height:"100%" }}>
                <rect width="600" height="400" fill="#3f5230"/>
                <polygon points="0,0 320,0 280,150 0,180" fill="#4c6238"/>
                <polygon points="320,0 600,0 600,120 280,150" fill="#5c6f40"/>
                <polygon points="0,180 280,150 250,280 0,320" fill="#52663a"/>
                <polygon points="280,150 600,120 600,260 250,280" fill="#445733"/>
                <polygon points="0,320 250,280 320,400 0,400" fill="#5e7344"/>
                <polygon points="250,280 600,260 600,400 320,400" fill="#3c4d2c"/>
                {[[80,70],[140,55],[110,120],[200,90],[60,160],[260,60],[230,140],[170,200],[300,110],[90,240],[180,290],[260,230],[340,180],[420,90],[480,140],[540,80],[450,210],[520,250],[400,300],[480,330],[560,300],[120,330],[220,350],[330,320]].map(([cx,cy],i)=>(
                  <circle key={i} cx={cx} cy={cy} r={13+ (i%4)*3} fill="#2f4524" opacity="0.8"/>
                ))}
                {/* analysis grid overlay in brand purple */}
                {[0,1,2,3,4,5].map(r=>(<line key={"h"+r} x1="0" y1={r*70} x2="600" y2={r*70} stroke="#a78bfa" strokeOpacity="0.28" strokeWidth="1"/>))}
                {[0,1,2,3,4,5,6,7,8].map(c=>(<line key={"v"+c} x1={c*70} y1="0" x2={c*70} y2="400" stroke="#a78bfa" strokeOpacity="0.28" strokeWidth="1"/>))}
                <polygon points="120,90 300,70 360,200 200,260 90,200" fill="rgba(124,58,237,0.22)" stroke="#7c3aed" strokeWidth="3" strokeDasharray="8 5"/>
              </svg>
              <div style={{ position:"absolute", top:12, left:12, display:"inline-flex", alignItems:"center", gap:6, background:"rgba(17,24,39,0.72)", color:"#fff", borderRadius:8, padding:"0.3rem 0.65rem", fontFamily:"'Inter', sans-serif", fontSize:"0.66rem", fontWeight:600 }}>
                <Scan style={{ width:13, height:13 }}/> Canopy analysis · LiDAR + Sentinel-2
              </div>
              <div style={{ position:"absolute", bottom:12, left:12, display:"inline-flex", alignItems:"center", gap:6, background:C.primary, color:"#fff", borderRadius:9999, padding:"0.32rem 0.8rem", fontFamily:"'Inter', sans-serif", fontSize:"0.72rem", fontWeight:600, boxShadow:"0 4px 12px rgba(124,58,237,0.4)" }}>
                <CheckCircle style={{ width:13, height:13 }}/> Canopy Cover: 84% · Avg Height: 14.2m
              </div>
            </div>
            <div>
              <Eyebrow>Built to last</Eyebrow>
              <H as="h2" style={{ fontSize:"2.2rem", marginTop:14 }}>Bring Unprecedented Transparency to Forest Carbon.</H>
              <Body style={{ marginTop:16, fontSize:"0.95rem" }}>Move beyond manual sampling. We combine remote sensing with field data to accurately track biomass growth, verify canopy cover, and ensure transparent payouts to local communities and smallholder farmers.</Body>
              <div style={{ marginTop:22, display:"flex", flexDirection:"column", gap:11 }}>
                {["Satellite & LiDAR remote sensing","Localized allometric equations","Direct-to-farmer payout ledger"].map(c=>(
          ...
          <div style={{ background:"#faf9ff", border:`1px solid ${C.pillBorder}`, borderRadius:22, padding:"2.8rem" }}>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"3rem", alignItems:"center" }}>
              <div>
                ...
                <H as="h2" style={{ marginTop:18, fontSize:"2.2rem" }}>Are you a <span style={{ background:grad, WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>project developer</span> or NGO?</H>
                <Body style={{ marginTop:14, fontSize:"0.9rem" }}>Indian agroforestry and reforestation projects can now earn credits under the mandatory <strong style={{ color:C.h }}>Carbon Credit Trading Scheme (CCTS) 2023</strong> under India's Energy Conservation Act.</Body>
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"0.5rem 1rem", marginTop:18 }}>
                  {["Unified data for CCTS & international registries","Verra, Gold Standard & Plan Vivo aligned","One-click validation & verification trail","NABARD & MoEFCC aligned reporting"].map(pt => (
...
function CarbonCredits() {
  const steps = [
    { icon:Trees,      tag:"GENERATE",    title:"Issue verified credits", desc:"Convert audited canopy and biomass data into issuance-ready carbon credits aligned with Verra (VM0047), Gold Standard, and Plan Vivo methodologies." },
...
const FEATURES = [
  ["High-integrity methodologies", "Built for Verra, Gold Standard & Plan Vivo, updated as standards evolve."],
  ["QR & API-based access",        "Each project links to its digital MRV record via secure, shareable access."],
  ["Automated carbon calculations","Sequestration and AGB computed automatically from verified inputs."],
  ["Real-time canopy monitoring",  "Live dashboards for canopy cover and biomass across every project."],
  ["Satellite & LiDAR integration","Plug-and-play connectors for remote-sensing and ground-truth data."],
  ["Audit-ready trails",           "Tamper-evident records that stand up to third-party validation."],
];
...
function Footer() {
  const ecoLinks = [
    ...
    { icon:TrendingUp, title:"Visit the Money Market",         sub:"Buy, sell & trade carbon credits", to:"carbon-credits" },
    { icon:FileText,   title:"What is Agroforestry dMRV?",      sub:"Read the introductory guide" },
    { icon:Activity,   title:"Strategic Implications for 2027", sub:"Comprehensive guide for project developers" },
  ];
  const contacts = [
    ...
    <footer style={{ background:C.footerBg, color:"#fff" }}>
      <div style={{ maxWidth:1180, margin:"0 auto", padding:"4rem 2rem" }}>
        ...
        <div style={{ marginTop:44, paddingTop:18, borderTop:"1px solid rgba(255,255,255,0.09)", display:"flex", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
          ...
          <p style={{ fontFamily:"'Inter', sans-serif", fontSize:"0.78rem", color:"rgba(199,185,255,0.45)", margin:0 }}>Agroforestry dMRV platform — originate, monitor, issue.</p>
        </div>
      </div>
    </footer>
  );
